...
export async function generateDeck(prompt: DeckPrompt, onGenerateCard?: (card: game.Card) => void): Promise<game.Card[]> {
  const cards: game.Card[] = []

  async function makeCard(attributes: string[], attack: number, health: number): Promise<void> {
...
export async function generateCard(prompt: CardPrompt): Promise<game.Card> {
  ...
  const attributesList = prompt.attributes.map(attr => `"${attr}"`).join(', ')

  const name = await createTextCompletion(prelude + `Write the name of a new card with theme "${prompt.theme}" and attributes ${attributesList}`)

  const abilityString = await createTextCompletion(prelude + `Write the ability of a new card with the theme "${prompt.theme}", attributes ${attributesList}, and name "${name}"`)

  const ability = parseAbility(abilityString)

  const abilityDescription = await createTextCompletion(prelude + `Write an exciting description of the ability "${abilityString}" of the card named "${name}" with theme "${prompt.theme}"`)

  const imagePrompt = await createTextCompletion(prelude + `Write a vivid description of the art for a card with the theme "${prompt.theme}", attributes ${attributesList}, name "${name}", and ability "${ability}"`)
